...
use pi_async::rt::{worker_thread::WorkerRuntime, AsyncRuntime, AsyncRuntimeBuilder, AsyncValue};
...
impl App {
    ...
    fn get_next_event_id(&self) -> EventID {
        self.0.borrow_mut().current_event_id += 1;
        EventID(self.0.borrow().current_event_id)
    }

    #[inline]
    fn get_next_animation_frame_id(&self) -> AnimationFrameID {
        self.0.borrow_mut().current_raf_id += 1;
        AnimationFrameID(self.0.borrow().current_raf_id)
    }
...
impl App {
    ...
    pub(crate) fn spawn_animation_frame() {
        let _ = APP_RUNTIME.spawn(APP_RUNTIME.alloc(), async move {
            log::info!("app_runtime spawn_animation_frame");

            let app = unsafe { APP_CONTEXT.as_ref().unwrap() };

            let on_raf = &mut app.0.borrow_mut().on_raf;
            
            let on_raf = on_raf.replace(XHashMap::default()).unwrap();
